...
ON DELETE CASCADE,PayPeriodStartDate date,PayPeriodEndDate date,Basicsalary decimal(10,2),OvertimePay decimal(10,2)
,Deductions decimal(10,2),Netsalary as(Basicsalary+OvertimePay-Deductions)persisted);

create table Tax(tax_id int primary key identity(201,1),employee_id int foreign key references Employee(employee_id)ON DELETE CASCADE,Taxyear int,
TaxableIncome decimal(10,2),TaxAmount decimal(10,2));

create table FinancialRecord(record_id int primary key identity(301,1),employee_id int foreign key references Employee(employee_id) ON DELETE CASCADE,
